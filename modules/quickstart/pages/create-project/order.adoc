...
To start the order process, switch to the _Order PCB_ panel (kbd:[F12]):
...
archive (menu:Project[Export *.lppz Archive]) and then upload
...
you like. For this, open the *Output Jobs* dialog from the sidebar (kbd:[F11]):

image::output-jobs-button.png[alt="Output Jobs Button"]
...
tools like https://gerber.ucamco.com/[the reference Gerber viewer] (preferred)
or http://gerbv.geda-project.org[gerbv]. LibrePCB developers are not
...
*Bill Of Materials (+*+.csv)*, and/or the interactive HTML BOM if you plan
to assembly your board by hand:
