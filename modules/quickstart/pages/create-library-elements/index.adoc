...
Open that library now in the libraries panel:

image::libraries-open.png[alt="Open Library Editor"]

In the library editor, there are six buttons to create the six different kinds
of library elements which LibrePCB is based on (no worries, important are only
four of them). So every new library element you will create during this
tutorial, starts with one of those buttons:
