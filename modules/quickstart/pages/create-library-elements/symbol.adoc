...
Now we need to create a symbol for the OpAmp. Click on btn:[New Symbol] in the
library editor, choose a name and the component category we just created and
click btn:[Next]:
...
_Note that to get a bigger working area, we collapsed the side panel by
clicking on the currently active panel button in the sidebar._
