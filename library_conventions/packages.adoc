...
* *Size information must use metric units*, not imperial units. For packages
  which are well known by their size in imperial units (e.g. "0805" which is
...
* For packages which are available with different pin counts, *append the pin
  ...
  of "DIP08").
* For manufacturer-specific packages, start with the manufacturers name (e.g.
  "Molex 53261-06"). _Note: Libraries do not act as namespaces for Package
