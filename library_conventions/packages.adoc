...
of it. Since LibrePCB handles footprints different than other EDA tools,
...
relevant for this decision. Even if a package can be mount differently on
a PCB (e.g. a THT resistor can be mount horizontally or vertically) and thus
require different footprints, only one Package is needed.
====
...
.Height (e.g. SO-8)
====
Some packages are available with different heights, e.g. SO-8 is available with
...
====
Many packages can be mount either vertically or horizontally, for example the
...
mounting variants should be handled by different footprints inside the Package.
====
...
Following conventions apply to Package names:
...
* *If {ipc7351-naming-pdf}[IPC-7351] specifies a name for a given package, use
  ...
  (like "SOT23-5") should be added to the comma-separated keywords list.
* *Size information must be given in millimeters*, not inches or mils. For
  packages which are well known by their size in inches (e.g. "0805" which is
  "2012" in metric), it's recommended to write the well known name in
  parentheses. For example a resistor could be called "RESC2012X70 (0805)".
...
* *Always add all pads of Packages*, even those which are not connected in
  some parts! For example the package "TO220" has a metal tab, so you should
...
to support following use-cases:

* *Mounting variants*: For example a THT resistor can be mount either
  vertically or horizontally with various pad distance. Every mounting
...
all three density levels as defined in IPC-7351 and can be mount either
