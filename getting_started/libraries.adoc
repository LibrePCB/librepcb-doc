...
and unique names!), add the `{{NAME}}` and `{{VALUE}}` variables and draw the
outline. The result should look like this:
...
- Add the `{{NAME}}` and `{{VALUE}}` variables. Most symbols should have exactly
  these two placeholders -- no more, no less.
